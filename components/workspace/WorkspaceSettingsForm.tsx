...
export function WorkspaceSettingsForm({
  ...
  return (
    <div className="space-y-8">
      {/* Response Style */}
      <div>
        <label className="block text-sm font-medium text-paper-dim mb-3">Response Style</label>
      ...
      {/* Language */}
      <div>
        <label className="block text-sm font-medium text-paper-dim mb-3">Response Language</label>
        <select
          value={settings.language}
          onChange={(e) => setSettings((s) => ({ ...s, language: e.target.value }))}
          className="w-full max-w-xs bg-ink-soft border border-paper/10 rounded-xl px-3 py-2 text-sm text-paper focus:outline-none focus:border-gold/50"
        >
      ...
      <div>
        <div className="flex items-center justify-between max-w-sm">
          <div>
            <label className="block text-sm font-medium text-paper-dim">Strict Mode</label>
          ...
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-ink-raised shadow transition-transform ${
      ...
      <div className="pt-2">
        <button
          onClick={handleSave}
          disabled={saving}
          className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gold hover:bg-gold-soft disabled:opacity-50 text-ink text-sm font-medium transition-colors"
        >
          <Save size={14} />
